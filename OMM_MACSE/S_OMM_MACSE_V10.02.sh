...
function quit_pb_option() {
    ...
    printf "\n usage example:\n$SCRIPT_NAME --out_dir out_dir --out_file_prefix out_file_prefix --in_seq_file seq_file.fasta [--genetic_code_number code_number] [--alignAA_soft MAFFT/MUSCLE] [--min_percent_NT_at_ends 0.7] [--out_detail_dir SAVE_DETAILS/] [--in_seq_lr_file less_reliable_seq_file.fasta] [--java_mem 500m] [--no_prefiltering] [--no_FS_detection] [--no_filtering] [--no_postfiltering] [--replace_FS_by_gaps] [--save_details]\n"
...
while (( $# > 0 )); do
    case "$1" in
       ...
       --java_mem)                   JAVA_MEM="$2"; shift 2                                   || quit_pb_option ; shift 2;;
       --out_dir)                    OUT_DIR=$(get_out_file_param "$1" "$2")                  || quit_pb_option ; shift 2;;
       --out_file_prefix)            OUT_FILE_PREFIX="$2"; shift 2                            || quit_pb_option ; shift 2;;
       --min_percent_NT_at_ends)     MIN_PERCENT_NT_AT_ENDS="$2";  shift 2                    || quit_pb_option ; shift 2;;
       --out_detail_dir)             OUT_DETAIL_DIR=$(get_out_file_param "$1" "$2")           || quit_pb_option ; SAVE_DETAILS=1; shift 2;;
       --no_prefiltering)            PRE_FILTERING=0;                                         || quit_pb_option ; shift 1;;
       --no_FS_detection)            FS_DETECTION=0;                                          || quit_pb_option ; shift 1;;
       --no_filtering)               FILTERING=0;                                             || quit_pb_option ; shift 1;;
       --no_postfiltering)           POST_FILTERING=0;                                        || quit_pb_option ; shift 1;;
       --replace_FS_by_gaps)         REPLACE_FS_GAP=1;                                        || quit_pb_option ; shift 1;;
       ...
       --alignAA_soft)               ALIGN_SOFT="$2";  || quit_pb_option; shift 2
           ...
           ;;
       --aligner_extra_option)       ALIGNER_EXTRA_OPTION="$2"                                || quit_pb_option ; shift 1;;
...
if [ -z ${out_file_prefix+x} ]; then printf "mandatory --out_file_prefix is missing"; quit_pb_option; fi
...
if(( FS_DETECTION > 0 )); then
  ...
  $macse -prog alignSequences $GC_OPT -seq __${PREFIX}_homol_tmp_NT.fasta -optim 2 -max_refine_iter 3 -local_realign_init 0.2 -out_NT __${PREFIX}_homol_tmp_NT.aln -out_AA __${PREFIX}_homol_tmp_AA.aln
  #check if it is fine or not
  if [ ! -f __${PREFIX}_homol_tmp_NT.aln ] ; then
      echo "Something goes wrong with MACSE alignSequences subprogram. This is generally link to memory problems. Try to re-launch the script with extra memory using the --java_mem option: e.g. --java_mem 2000m "
      exit 1;
  fi
  #prepare fasta file
