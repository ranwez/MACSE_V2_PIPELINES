...
function quit_pb_option() {
    ...
    printf "\n\nFor further details please check the documentation on MACSE website: https://www.agap-ge2pop.org/macse/ \n\n"
    exit 1
}
...
ALIGN_SOFT="MAFFT"; 
# ALIGNER_EXTRA_OPTION="" # should not be in or PRANKitialized at all for test [ -z ${ALIGNER_EXTRA_OPTION+x}  ] to work 
SAVE_DETAILS=0; debug=0;JAVA_MEM=250m
MIN_PERCENT_NT_AT_ENDS=0.7; MIN_SEQ_TO_KEEP_SITE=0; MIN_MEM_OPT=""
    ...
    case "$1" in
       ...
       --alignAA_soft)               ALIGN_SOFT="$2"                                           || quit_pb_option; shift 2
           if [[ ! "$ALIGN_SOFT" =~ ^(MAFFT|MUSCLE|PRANK)$ ]];  then
             echo " Alignment software $ALIGN_SOFT is not handle, please choose between MAFFT, MUSCLE or PRANK"; quit_pb_option
           fi
           ;;
...
cd $tmp_dir || { printf "problem to create tmp directoty or cd to $tmp_dir\n" ; exit 1; }
...
if(( $PRE_FILTERING > 0)); then
    printf "\n\n============== MACSE PRE_FILTERING\n"
    if (( $HAS_SEQ_LR > 0 )); then
        $macse -prog trimNonHomologousFragments $GC_OPT $MIN_MEM_OPT -seq __all_seq.fasta -out_NT __${PREFIX}_homol_tmp_NT_all.fasta -out_AA __${PREFIX}_homol_tmp_all_AA.fasta -out_trim_info __${PREFIX}_homol_fiter.csv -out_mask_detail __${PREFIX}_NonHomolFilter_NT_mask_detail.fasta -min_trim_in 60 -min_trim_ext 45 -debug
      ...
      $macse -prog trimNonHomologousFragments $GC_OPT $MIN_MEM_OPT -seq __all_seq.fasta -out_NT __${PREFIX}_homol_tmp_NT.fasta -out_AA __${PREFIX}_homol_tmp_AA.fasta -out_trim_info __${PREFIX}_homol_fiter.csv -out_mask_detail __${PREFIX}_NonHomolFilter_NT_mask_detail.fasta -min_trim_in 60 -min_trim_ext 45 -debug
    fi
...
if (( SAVE_DETAILS > 0 )); then
    printf "\n\n==============  saving details \n\n"
    if [ -z "${OUT_DETAIL_DIR+x}" ]; then
        DETAIL_DEST="$OUT_DIR/details_$PREFIX"
    else
        DETAIL_DEST="$OUT_DETAIL_DIR"
    fi

    mkdir -p "$DETAIL_DEST"
    cp -r "$tmp_dir"/* "$DETAIL_DEST"/
fi
printf "\n\n==============  $PREFIX processed\n\n"